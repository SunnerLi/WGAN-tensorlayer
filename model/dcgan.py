...
class DCGAN(object):
    def __init__(self, img_channel, filter_base = 64, fc_unit_num = 1024, conv_depth = 4, lambda_panelty_factor = 10.0, name='wgan_'):
    ...
    def build(self, noise_ph, image_ph):
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            self.generator_optimize = tf.train.AdamOptimizer(learning_rate=0.01, beta1=0.5, beta2=0.9).minimize(self.generator_loss, var_list=generator_vars)
            self.discriminator_optimize = tf.train.AdamOptimizer(learning_rate=0.001, beta1=0.5, beta2=0.9).minimize(self.discriminator_loss, var_list=discriminator_vars)
    ...
    def getDiscriminator(self, img_ph, reuse=False):
        with tf.variable_scope(self.name + 'discriminator', reuse=reuse):
            ...
            for i in range(self.conv_depth):
                ...
                network = tl.layers.Conv2d(network, n_filter = channel, name ='discriminator_conv2d_%s'%str(i))
    ...
    def getGenerator(self, noise_ph):
        ...
        with tf.variable_scope(self.name + 'generator'):
            ...
            for i in range(self.conv_depth, 1, -1):
                ...
                network = tl.layers.DeConv2d(network, n_out_channel = channel, strides = (2, 2), out_size = (height, width), name ='generator_decnn2d_%s'%str(self.conv_depth-i))
