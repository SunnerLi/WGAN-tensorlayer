...
def export_images(db_path, out_dir, flat=False, limit=10):
    ...
    with env.begin(write=False) as txn:
        ...
        for key, val in cursor:
            if not flat:
                img_name = key[:6].decode('utf-8')
                image_out_dir = out_dir + '/' + (img_name)
            else:
                image_out_dir = out_dir
            print('\n\nimage_out_dir:', image_out_dir)

            image_out_path = image_out_dir + img_name + '.webp'
